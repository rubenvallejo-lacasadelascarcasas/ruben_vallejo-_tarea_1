...
class Bizum extends PaymentModule
{
	private	$html = '';
	...
	public function __construct()
	{
		...
		if (isset($config['BIZUM_NOMBRE']))
			$this->nombre = $config['BIZUM_NOMBRE'];
		if (isset($config['BIZUM_CODIGO']))
			$this->codigo = $config['BIZUM_CODIGO'];
		if (isset($config['BIZUM_TERMINAL']))
			$this->terminal = $config['BIZUM_TERMINAL'];
		if (isset($config['BIZUM_CLAVE256']))
			$this->clave256 = $config['BIZUM_CLAVE256'];
		//if (isset($config['BIZUM_TRANS']))
		//	$this->trans = $config['BIZUM_TRANS'];
		if (isset($config['BIZUM_ERROR_PAGO']))
			$this->error_pago = $config['BIZUM_ERROR_PAGO'];
		if (isset($config['BIZUM_LOG']))
			$this->activar_log = $config['BIZUM_LOG'];
		if (isset($config['BIZUM_IDIOMAS_ESTADO']))
			$this->idiomas_estado = $config['BIZUM_IDIOMAS_ESTADO'];;
		if (isset($config['BIZUM_ESTADO_PEDIDO']))
			$this->estado_pedido = $config['BIZUM_ESTADO_PEDIDO'];
					
		parent::__construct ();
		
		$this->page = basename(__FILE__, '.php');
		$this->displayName = $this->l('Bizum');
		$this->description = $this->l('Aceptar pagos mediante Bizum');
		...
		$this->warning = $this->l('Faltan datos por configurar del módulo de Bizum.');
	}

	
	public function install(){
		if (!parent::install()
				|| !Configuration::updateValue('BIZUM_URLTPV', '0')
				|| !Configuration::updateValue('BIZUM_NOMBRE', $this->l('Escriba el nombre de su tienda'))
				|| !Configuration::updateValue('BIZUM_TERMINAL', 1)
				|| !Configuration::updateValue('BIZUM_CLAVE256', $this->l('Escriba la clave de su tienda'))
				|| !Configuration::updateValue('BIZUM_TRANS', 0)
				|| !Configuration::updateValue('BIZUM_ERROR_PAGO', 'no')
				|| !Configuration::updateValue('BIZUM_LOG', 'no')
				|| !Configuration::updateValue('BIZUM_IDIOMAS_ESTADO', 'no') 
				|| !Configuration::updateValue('BIZUM_ESTADO_PEDIDO', '2' )
	...
	private function tratarJSON(){
		...
		if(file_exists($fileName) &&  _PS_VERSION_ >= 1.7){
			...
			if ( $jsonDecode[bizum] != null && $jsonDecode[bizum][name] != null){
				$jsonDecode[bizum][name]="ps_bizum";
	...
	public function uninstall()	{
		...
		if (!Configuration::deleteByName('BIZUM_URLTPV')
			|| !Configuration::deleteByName('BIZUM_NOMBRE')
			|| !Configuration::deleteByName('BIZUM_CODIGO')
			|| !Configuration::deleteByName('BIZUM_TERMINAL')
			|| !Configuration::deleteByName('BIZUM_CLAVE256')
			|| !Configuration::deleteByName('BIZUM_TRANS')
			|| !Configuration::deleteByName('BIZUM_ERROR_PAGO')
			|| !Configuration::deleteByName('BIZUM_LOG')
			|| !Configuration::deleteByName('BIZUM_IDIOMAS_ESTADO')
			|| !Configuration::deleteByName('BIZUM_ESTADO_PEDIDO')
			|| !parent::uninstall())
			return false;
		return true;
	}
	...
	private function _postProcess()	{
		...
		if (Tools::isSubmit('btnSubmit')){
			Configuration::updateValue('BIZUM_URLTPV', Tools::getValue('urltpv'));
			Configuration::updateValue('BIZUM_NOMBRE', Tools::getValue('nombre'));
			Configuration::updateValue('BIZUM_CODIGO', Tools::getValue('codigo'));
			Configuration::updateValue('BIZUM_TERMINAL', Tools::getValue('terminal'));
			Configuration::updateValue('BIZUM_CLAVE256', Tools::getValue('clave256'));
			//Configuration::updateValue('BIZUM_TRANS', Tools::getValue('trans'));
			Configuration::updateValue('BIZUM_ERROR_PAGO', Tools::getValue('error_pago'));
			Configuration::updateValue('BIZUM_LOG', Tools::getValue('activar_log'));
			Configuration::updateValue('BIZUM_IDIOMAS_ESTADO', Tools::getValue('idiomas_estado'));
			Configuration::updateValue('BIZUM_ESTADO_PEDIDO', Tools::getValue ( 'estado_pedido' ) );
		}
	...
	private function _displayBizum()
	{
		// lista de payments
		$this->html .= '<img src="../modules/bizum/img/bizum.png" style="float:left; margin-right:15px;"><b><br />'
		.$this->l('Este módulo le permite aceptar pagos por Bizum.').'</b><br />'
		...
		$this->html .= '<form action="'.$_SERVER['REQUEST_URI'].'" method="post">
			...
			<legend><img src="../img/admin/contact.gif" />'.$this->l('Configuración del TPV').'</legend>
				<table border="0" width="680" cellpadding="0" cellspacing="0" id="form">
					...
					<tr><td width="255" style="height: 35px;">'.$this->l('Entorno de Bizum').'</td><td><select name="urltpv"><option value="1"'.$entorno_real.'>'.$this->l('Real').'</option><option value="2"'.$entorno_t.'>'.$this->l('Pruebas en sis-t').'</option><option value="3"'.$entorno_i.'>'.$this->l('Pruebas en sis-i').'</option><option value="4"'.$entorno_d.'>'.$this->l('Pruebas en sis-d').'</option></select></td></tr>
	...
	private function createParameter($params){
		...
		// if (isset ( $_COOKIE ["bizumP" . $orderId] )) {
		// 	$sec_pedido = $_COOKIE ["bizumP" . $orderId];
		// } else {
		// 	$sec_pedido = - 1;
		// }
		// $logActivo = "si";
		// escribirLog ( " - COOKIE: " . $_COOKIE ["bizumP" . $orderId] . "($orderId) - secPedido: $sec_pedido", $logActivo );
		// if ($sec_pedido < 9) {
		// 	setcookie ( "bizumP" . $orderId, ++ $sec_pedido, time () + 86400 ); // 24 horas
		// }
		// $numpedido = str_pad ( $orderId . $sec_pedido, 12, "0", STR_PAD_LEFT );

		$valorCookie=$this->getBizumCookie($orderId);
		...
		if ($sec_pedido < 9) {
			$this->setBizumCookie($orderId,++ $sec_pedido);
		}
		...
		if (empty ( $_SERVER ['HTTPS'] )) {
			$protocolo = 'http://';
			$urltienda = $protocolo . $_SERVER ['HTTP_HOST'] . __PS_BASE_URI__ . 'index.php?fc=module&module=bizum&controller=validation';
		} else {
			$protocolo = 'https://';
			$urltienda = $protocolo . $_SERVER ['HTTP_HOST'] . __PS_BASE_URI__ . 'index.php?fc=module&module=bizum&controller=validation';
		}
		...
		if ($idiomas_estado == 'si') {
			...
			$miObj->setParameter ( "Ds_Merchant_Module", "op_bizum_" . $this->version );
	...
	public function hookDisplayPaymentEU($params){	
        ...
        return array(
            'cta_text' => "Pagar con Bizum",
            'logo' => _MODULE_DIR_."bizum/img/bizum.png",
	...
	public function getBizumCookie($order){
		$key="bizum".str_pad($order,12,"0",STR_PAD_LEFT);
	...
	public function setBizumCookie($order,$valor){
		$key="bizum".str_pad($order,12,"0",STR_PAD_LEFT);
